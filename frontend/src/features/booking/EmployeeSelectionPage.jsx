...
const EmployeeSelectionPage = () => {
  ...
  const handleContinue = () => {
    ...
    navigate('/booking/datetime', { 
      state: { vehicleId, serviceIds, locationId, employeeId: selectedEmployeeId } 
    })
  }
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="container mx-auto px-4 py-12 space-y-8">
        <div className="space-y-2">
          ...
          <p className="text-gray-600">Choose a specific employee or let us assign the best available.</p>
        </div>
            ...
            >
              <Card
                className={cn(
                  ...
                  selectedEmployeeId === employee.id && 'border-blue-500 shadow ring-2 ring-blue-500 ring-offset-0'
                )}
              >
                <CardHeader>
                  <CardTitle className="flex items-start gap-3 text-lg">
                    <span className={cn(
                      ...
                      {employee.isDefault ? <Users size={20} /> : <User size={20} />}
                    </span>
                    <div className="flex-1">
                      <div>{employee.name}</div>
                      <div className="text-sm font-normal text-gray-600 mt-1">{employee.role}</div>
                    </div>
                  </CardTitle>
                </CardHeader>
                <CardContent>
