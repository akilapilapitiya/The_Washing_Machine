  ...
  <Link to={to} className="block">
    <Card className="h-full border-gray-100 hover:border-red-200 hover:shadow-md transition-all group overflow-hidden bg-white">
      <CardContent className="p-4 sm:p-6 flex flex-col items-center justify-center text-center gap-3">
        ...
        <p className="font-bold text-gray-900 text-sm group-hover:text-red-700 transition-colors">{title}</p>
      </CardContent>
    </Card>
  </Link>
);
...
const EmployeeDashboard = ({ data, loading }) => {
  ...
  const progressPercent = totalAssignedToday > 0 ? Math.round((completedJobs / totalAssignedToday) * 100) : 0;
      ...
      <div className="space-y-6">
         <div className="flex items-center justify-between">
            ...
            <Link to="/dashboard/employee/assigned">
               <Button variant="ghost" className="text-red-700 hover:text-red-800 hover:bg-red-50 font-semibold">
                  View Full Queue
               </Button>
            </Link>
         </div>
         ...
         {loading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
               {[1, 2, 3].map(i => <div key={i} className="h-40 bg-white border border-gray-100 animate-pulse rounded-2xl shadow-sm"></div>)}
            </div>
         ) : assignedJobs.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
               {assignedJobs.slice(0, 6).map((job) => (
                  <Card key={job.bookingid} className="border-gray-200 shadow-sm hover:shadow-md hover:border-red-200 transition-all group overflow-hidden bg-white">
                     <CardContent className="p-6">
                        <div className="flex justify-between items-start mb-4">
                           <div>
                              ...
                              <h3 className="font-bold text-gray-900 text-lg leading-tight truncate pr-4" title={job.servicename || "Service Job"}>
                                 {job.servicename || "Service Job"}
                              </h3>
                           </div>
                           <span className="text-xs font-bold text-gray-400 shrink-0">#{job.bookingid}</span>
                        </div>
                        ...
                        <Button asChild className="w-full bg-red-50 text-red-700 hover:bg-red-600 hover:text-white font-bold transition-colors">
                           <Link to={`/dashboard/employee/assigned`}>
            ...
            <Card className="border-dashed border-2 border-gray-200 bg-gray-50/50">
               <CardContent className="p-12 text-center">
                  ...
                  <p className="text-gray-500 max-w-sm mx-auto">You have no active or pending jobs assigned to you at the moment. Enjoy your break!</p>
               </CardContent>
            </Card>
         )}
      </div>
