...
const ManageDailySchedulePage = () => {
  ...
  const fetchDayData = async (dateStr) => {
    try {
      ...
      const branchSchedule =
        await schedulerService.getBranchDailySchedule(dateStr);

      // 2. Fetch partial holidays (blocked slots) for this date
      const holidayData = await holidayService.getHolidaysByRange(
        dateStr,
        dateStr,
      );
  ...
  const isSlotBooked = (slotTime) => {
    ...
    return bookings.some((b) => {
      ...
      return !(
        b.scheduleendtime <= slotStart || b.schedulestarttime >= slotEnd
      );
    });
  };
  ...
  const handleSave = async () => {
    try {
      ...
      const slots = Array.from(blockedSlots)
  ...
  const rosterColumns = [
    {
      ...
      label: "Time Slot",
      render: (b) =>
        `${b.schedulestarttime.substring(0, 5)} - ${b.scheduleendtime.substring(0, 5)}`,
    },
    {
      key: "customer",
      label: "Customer",
    ...
    },
    {
      ...
      label: "Assigned To",
      render: (b) =>
        b.employee_firstname ? (
          `${b.employee_firstname} ${b.employee_lastname}`
        ) : (
            ...
            <div>
              <div className="mb-4">
                ...
                <p className="text-xs text-gray-500">
                  Pick the date you want to adjust timeslots for.
                </p>
              </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-[10px] font-medium mt-0.5 opacity-80 flex items-center gap-1">
                            {booked ? (
                              <>
                                <Lock size={10} className="inline" /> Booked
                              </>
                            ) : isBlocked ? (
                              "Blocked"
                            ) : (
          ...
          <Card className="border border-gray-200 shadow-sm overflow-hidden mt-6">
            <div className="bg-gray-50 px-5 py-4 border-b border-gray-200">
              <h2 className="text-sm font-bold text-gray-900">
                Booked Customers Roster
              </h2>
              <p className="text-xs text-gray-500 mt-0.5">
                Contact customers instantly if operational delays occur today.
              </p>
            </div>
