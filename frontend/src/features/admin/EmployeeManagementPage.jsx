...
import { Users, Plus, Trash2, TrendingUp, Mail, Phone, Badge, AlertCircle, CheckCircle, X } from 'lucide-react'
...
const levelColors = {
  junior: { bg: 'bg-blue-100', text: 'text-blue-800', border: 'border-blue-300' },
  mid: { bg: 'bg-purple-100', text: 'text-purple-800', border: 'border-purple-300' },
  senior: { bg: 'bg-green-100', text: 'text-green-800', border: 'border-green-300' },
  lead: { bg: 'bg-orange-100', text: 'text-orange-800', border: 'border-orange-300' },
  ...
  const handlePromoteEmployee = (e) => {
    ...
    const updatedEmployees = employees.map(emp =>
      emp.id === selectedEmployee.id
        ? { ...emp, level: newRole, role: roleOptions.find(r => r.value === newRole)?.label }
        : emp
    )
      ...
      <div className="container mx-auto px-4 py-12 space-y-8">
        <div className="flex items-start justify-between">
          <div className="space-y-2">
            <p className="text-sm uppercase tracking-wide text-blue-600 font-semibold">Employee Management</p>
            <h1 className="text-3xl font-bold">Manage Employees</h1>
            <p className="text-gray-600">Add, promote, and manage your team members.</p>
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="text-center">
                <p className="text-3xl font-bold text-green-600">{employees.filter(e => e.level === 'senior' || e.level === 'master' || e.level === 'lead').length}</p>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="text-center">
                <p className="text-3xl font-bold text-yellow-600">{employees.filter(e => e.level === 'mid').length}</p>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="text-center">
                <p className="text-3xl font-bold text-purple-600">{employees.filter(e => e.level === 'junior').length}</p>
                  ...
                  <CardContent className="space-y-3">
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="text-xs text-gray-600">
                        <span>Joined: {new Date(employee.joinDate).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}</span>
                      </div>
                    </div>
          ...
          <Card className="w-full max-w-md">
            <CardHeader>
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => setShowPromoteForm(false)} className="text-gray-500 hover:text-gray-700">
                  <X size={24} />
                </button>
              </div>
            </CardHeader>
            <CardContent>
              <form onSubmit={handlePromoteEmployee} className="space-y-4">
                <div>
                  ...
                  <p className="text-sm text-gray-600">Current: {roleOptions.find(r => r.value === selectedEmployee.level)?.label}</p>
                </div>
                <div className="space-y-2">
                ...
                {newRole && newRole !== selectedEmployee.level && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                    <p className="text-sm text-blue-800">
                      Promoting from <span className="font-semibold">{roleOptions.find(r => r.value === selectedEmployee.level)?.label}</span> to <span className="font-semibold">{roleOptions.find(r => r.value === newRole)?.label}</span>
                    </p>
                  </div>
                )}
                <div className="flex gap-3 justify-end pt-4">
                  <Button type="button" variant="outline" onClick={() => setShowPromoteForm(false)}>
                    Cancel
                  </Button>
                  <Button type="submit" disabled={newRole === selectedEmployee.level}>
                    Promote Employee
                  </Button>
                </div>
              </form>
