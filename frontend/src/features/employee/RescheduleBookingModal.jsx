  ...
  useEffect(() => {
    ...
    const fetchAvailability = async () => {
      try {
        ...
        if (booking.empid) {
          existingSchedules = await schedulerService.getEmployeeScheduleByDate(
            booking.empid,
            selectedDate,
          );
        } else {
          existingSchedules =
            await schedulerService.getBranchDailySchedule(selectedDate);
        }

        const partialHolidays = await holidayService.getHolidaysByRange(
          selectedDate,
          selectedDate,
        );
        ...
        setScheduleArray(
          existingSchedules
            ? existingSchedules.filter((s) => s.bookingid !== booking.bookingid)
            : [],
        );
      ...
      <Card className="w-full max-w-4xl shadow-2xl border-gray-200 rounded-xl overflow-hidden bg-white flex flex-col max-h-[90vh]">
        <div className="px-6 py-4 border-b border-gray-100 bg-gray-50 flex items-center justify-between shrink-0">
          <CardTitle className="text-sm font-bold flex items-center gap-2">
            <Calendar size={16} className="text-red-600" /> Administrative
            Rescheduling
          </CardTitle>
          <X
          ...
          <div className="grid lg:grid-cols-2 divide-y lg:divide-y-0 lg:divide-x divide-gray-100 min-h-0">
            ...
            <div className="p-6 bg-white">
              <div className="mb-4">
                ...
                <p className="text-xs text-gray-500">
                  Only future operational dates are permitted.
                </p>
              </div>
            ...
            <div className="p-6 bg-gray-50/50">
              <div className="mb-4">
                ...
                <p className="text-xs text-gray-500">
                  Timeslots are filtered against existing resource constraints.
                </p>
              </div>
        ...
        <div className="px-6 py-4 border-t border-gray-100 bg-white flex items-center justify-end gap-3 shrink-0">
          <Button
            variant="ghost"
            onClick={onClose}
            className="text-xs font-bold uppercase tracking-wider text-gray-500 hover:text-gray-900"
          >
            Cancel
          </Button>
          <Button
