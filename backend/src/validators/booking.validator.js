...
export const bookingValidator = {
  createBooking: Joi.object({
    ...
    bookinglocationlatitude: Joi.number().min(-90).max(90).required().messages({
  ...
  updateBooking: Joi.object({
    bookingstatus: Joi.string()
      ...
      .messages({
        "any.only":
          "Booking status must be one of: pending, inProgress, completed, paid",
      }),
    bookingdate: Joi.date().iso(),
    bookingstarttime: Joi.string().pattern(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/),
    bookingendtime: Joi.string().pattern(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/),
    bookinglocationlatitude: Joi.number().min(-90).max(90),
    bookinglocationlongitude: Joi.number().min(-180).max(180),
  }).min(1),
};
