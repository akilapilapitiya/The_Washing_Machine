 ...
 */
export const cacheMiddleware = (ttlSeconds = 3600) => async (req, res, next) => {
  ...
  try {
    ...
    logger.info({ cacheKey }, "Redis Edge Cache Miss. Fetching from Database.");
    ...
    res.json = (body) => {
      ...
      if (res.statusCode >= 200 && res.statusCode < 300) {
        redisClient.set(cacheKey, JSON.stringify(body), "EX", ttlSeconds)
          .catch(err => {
            logger.error({ err, cacheKey }, "Failed to persist Database response to Redis Edge Cache.");
          });
      }
    };
    ...
    logger.error({ error }, "Fatal Error in Redis Edge Caching Middleware. Falling back to primary Database.");
    next();
  }
};
