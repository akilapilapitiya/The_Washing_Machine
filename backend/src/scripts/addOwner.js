...
async function addOwner() {
  ...
  try {
    ...
    // Hash password
    const passwordHash = await bcrypt.hash(ownerData.password, Number(SALT_ROUNDS));
    ...
    const result = await pool.query(
      ...
      [ownerData.name, ownerData.email, ownerData.telephone, passwordHash, ownerData.type, ownerData.nic]
    );
